...
const Following = () => {
  ...
  const [followingList, setFollowingList] = useState([DEFAULT_DATA]);

  const getUserInfo = async (userId: string) => {
    try {
      setLoading(true);
      const { following } = await userInfo(userId); // 팔로우 아이디 받음
    ...
    <Container>
      {followingList ? (
        followingList.map((follow) => (
          <Wrapper key={follow._id}>
            <FollowList src={DUMMY_IMAGE} fullName='123' />
            <Button
              variant='outlined'
              size='small'
